...
#   Superseded by 05-research/scripts/tick.sh, which decides for itself what is due.
...
#
# Every run appends to 04-outputs/AA-evals/<date>/runbook-<stamp>.log and exits
...
step "1/5 refresh bars"      "$PY" 03-inputs/alpaca_data.py download
step "2/5 rebalance"         "$PY" 03-inputs/alpaca_trade.py rebalance --execute
step "3/5 catastrophe stop"  "$PY" 03-inputs/alpaca_trade.py check
step "4/5 mark to market"    "$PY" 03-inputs/alpaca_trade.py status
step "5/5 execution report"  "$PY" 03-inputs/alpaca_execution_report.py
