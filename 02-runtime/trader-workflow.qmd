...
This document was written for someone intelligent and well read who is not a trader and has never trained a model. That second half is the part that does the work, because it marks which words have to be introduced rather than assumed. Fee, label, frame, feature, base rate, out-of-sample, overfit and survivorship all mean something narrow here, and each is defined in the sentence that first uses it. Appendix B collects them.

The subject is easier to hold than the vocabulary suggests. A person wants to buy something, hold it for a while, and sell it for more than they paid. Everything below is an attempt to answer one question about that. Is there a rule for deciding what to buy and when to sell that makes money on days the rule has never seen, after the broker has taken its cut.

The cut is the whole difficulty, so it is worth a number straight away. Buying a thousand dollars of a coin on Binance and selling it again costs about two dollars in fees. The price has to rise more than two tenths of one per cent before a single cent has been made. Most of the patterns in this document move the price by less than that. They are real patterns, they are not worth trading, and the gap between those two statements is what took a year to establish.

## The scope

The work covers two markets and one method. The method is the same in both. Download every price bar ever recorded, turn each bar into a row of numbers describing what the market looked like at that moment, attach to each row what happened next, train a model to predict that, then hide the most recent year from the model completely and ask it once, on that hidden year, whether it made money after fees. One answer, scored once, no second attempt. The hiding is not a formality. A model shown its own test data will pass every test and lose every dollar.

The first market is spot cryptocurrency on Binance, meaning coins bought outright with cash rather than borrowed. It occupies most of the document, and every candidate it produced was rejected. Section 5 gives the current reading, which is worse than the fee story it replaced. On 7 September 2026 both surviving crypto frames lost money with the fees stripped out entirely, minus 0.182 per cent per trade on four-hour bars and minus 1.150 per cent on daily bars, so a year spent treating this as a fee problem was treating a symptom.

The second market is United States shares through a broker called Alpaca, introduced in section 0.4. One rule there passed every test, and it is running now on a practice account holding fifty companies. The switch that would let it spend real money is off and has never been on.

Three things this document is not. It is not advice, and nothing in it should be traded on. It is not finished, because the crypto search closed with a negative answer and the equity rule has completed three of the six clean weekly cycles it needs before its real trading costs can be trusted. It is not neutral about its own weaknesses, and the ones that would change a reader's mind are stated at the point they arise rather than collected in a footnote. The largest is that the share-price history covers only companies still trading today, so every company that went bankrupt is silently missing and every equity number below reads better than reality.
...
The higher-timeframe context shifts with the frame, so a five-minute model still sees the one-hour and four-hour trend, a one-hour model sees the four-hour and daily trend, and a four-hour model sees the daily and weekly trend. That is how each frame gets multi-resolution information without the datasets ever being merged. Every modelling section in Part 3 runs this same 1h, 4h, 5m loop and reports one block per frame. The 1d frame was added 2026-08-17 as the fee-count lever: the survivorship-complete daily archive (669 pairs) is on disk and its baseline dataset and edge matrix are section 3.13's first read.

### 0.4 Equity Data

The second venue is Alpaca, a US stockbroker, and it is where the only strategy that has ever passed the after-fee test is running today. It is introduced here beside the Binance archives because a reader who meets it only in an appendix will assume the crypto search is the whole project, and it is not. Crypto is where the method was built. Equities are where the method first found something.

The two data layers answer the same questions and differ in three ways that matter.
...
| Dead names | recovered, so results are honest | absent, so results are an upper bound |
...
That last row is the reason the equity track exists. Every crypto candidate died at the fee, not at the prediction. Moving to a venue where trading costs a twenty-fifth as much changes which weak signals are worth acting on, and a signal too weak to survive a 0.20 percent round trip can survive a 0.04 percent one.

**The survivorship problem, and which way it cuts.** Alpaca's asset list reports the companies trading today, so a firm that went bankrupt in 2019 is simply absent from the history. A backtest that never sees the failures reads better than reality. This is the opposite of the Binance discipline, where the archives were deliberately crawled to recover the delisted pairs. It is accepted for the equity read and never hidden. `03-inputs/alpaca_data.py` writes the caveat into the universe snapshot itself, and every record built on the panel repeats it. A separate stress test, `03-inputs/equity_survivorship_stress.py`, injects delistings and re-scores, and the edge held.
...
**One consequence of sharing the builder.** The equity features keep the crypto column names, so a column called `f_btc_rs` on the equity panel holds a stock's strength relative to SPY, not to Bitcoin. The names were left alone so that every tool downstream reads both panels without a translation layer. It is confusing on first sight and it is the deliberate choice.

**What the equity data found.** The pre-registered monthly factor test, `03-inputs/equity_momentum_monthly.py`, ranks names by their return over the prior twelve months excluding the most recent one, holds the top tenth for a month, and repeats without overlapping. Nothing in it is picked using this project's own model scores, which is what makes it a fair test rather than a fitted one. Over 116 months to 5 September 2026, after a 5 basis point monthly cost, the top tenth beat the market average by 1.006 percentage points per month, with a t-statistic of 2.41, an absolute return above zero in 85 percent of half-year folds and a spread above zero in 80 percent of them. That is the project's first and only SURVIVES. The same harness killed the six-month version and killed low volatility, exactly as the published literature says it should, which is the evidence that the harness is capable of returning a negative.

The honest counterweight is the most recent fold. In the first half of 2026 the top tenth beat the market by 5.319 points a month, the best reading in the series; in the three months since, it has trailed the market by 5.646 points a month, the worst. The live paper book was funded into the second of those. Section 5 carries what that has cost so far.

**Where it runs.** `03-inputs/alpaca_trade.py` holds the fifty highest-ranked names at 1.8 percent each and rebalances weekly. Never short and never on margin are enforced in the code, not left to the operator. The account is a paper account, so the money is fake, and `LIVE_TRADING` stays false. Appendix D and section 5 carry the running book.
...
4. **Longer-horizon frame.** Extend the search toward the daily frame alongside 4h, because coarser frames cut the fee count per unit of return, which is the central obstacle. As of 2026-08-17 the daily archive is complete and the baseline build is in flight (3.13).
...
The honest finding from Part 3 is that the entry rule alone, these trend signals fired as a long trigger, sits at the efficient-market floor on the 1h frame: the trend reverses back through the entry almost as often as it continues. That is why the search moved to cross-sectional ranking and a coarser frame rather than to more entry features. The board's top candidates below illustrate an entry read on daily candles; they are not the decision-grade entry.
...
_view = screen[cols]
print(f"\nfour-gate screen -- {int(screen['pass'].sum())} of {len(screen)} coins passed:")
print(_view.to_string(index=False))    # text table: renders in every editor and in the Word export
display(_view)
```
...
print("logged ->", p)
_log = pd.read_csv(p)
print(f'\nexperiment_log.csv ({len(_log)} rows; last 3):')
print(_log.tail(3).to_string(index=False))    # text fallback: renders in every IDE
display(_log.tail(3))
```
...
The 0.5 adjustments are implemented across these sections: the label in 3.1 and 3.7, the ATR-scaled exits in 3.3, the regime gate in 3.12, and the longer-horizon frame across every per-frame loop. The cells run top to bottom on the project `.venv`. `LOAD_FRAME` selects the active frame the single-frame cells consume; the multi-frame sections from 3.9 on loop over all three frames on their own and restore the active frame afterward.
...
# functions in 03-inputs/build_dataset_1h.py, wired into build_coin() beside the in-house / pandas-ta /
...
print(f'\nsymbol metadata (first 10 of {len(md_tbl)} rows):')
print(_view.to_string(index=False))          # text fallback: renders in every IDE
display(_view)
```
...
if df_wf is not None:
    ...
    print(f'\nwalk-forward folds ({len(_folds)}):')
    print(_folds.to_string(index=False))      # text fallback: renders in every IDE
    display(_folds)
...
ways, because no single measure is honest alone. A feature's own area under the curve says how well it
...
```{python}
#| echo: true
#| code-summary: "the feature evidence, run as a script"
# .venv/bin/python 03-inputs/feature_report.py --frames 4h 1d --rows 120000
print("computed by script; the record it wrote is rendered below.")
```
...
```{python}
#| echo: true
#| code-summary: "the two tuning sweeps, run as scripts"
# .venv/bin/python 03-inputs/sweep_label_1h.py    label geometry: target, stop, horizon
# .venv/bin/python 03-inputs/walkforward.py       exit geometry: stop, take-profit, trail
# .venv/bin/python 03-inputs/model_assessment_1h.py --tune   model hyperparameters
print("tuning is run from the shell; this section reads the records those runs persisted.")
```
...
the grid, which is what you would expect when the ceiling is set by the data rather than the model.
...
earned its position rather than drawn a kind fold.
...
```{python}
import model_metrics as mm   # heading demotion for embedded records
...
### 3.11 Edge Diagnostics

Why the scoreboard says NO-GO, in three questions plus the honest selectivity test, run per frame. Q1 asks whether a pre-cost edge exists at all against a base-rate coin flip and a one-bar persistence rule. Q5 asks whether any edge is stable across eras or an artifact of one regime. Q6 sweeps the confidence threshold to see whether selectivity pays. The out-of-sample selectivity test then chooses the threshold on train and scores it once on the blind year, so a thin in-sample peak cannot pass. The verdict is a genuine floor, not a leakage artifact; the leakage audit is clean.

```{python}
import model_metrics as mm   # heading demotion + file:// image links for embedded records
import pathlib
...
if os.path.exists(book_path):
    ...
    for r in book["book"]:
        print(f"  {r['pair']:<14} contrib {r['contrib_pct']:+6.2f}pp  win {r['win_rate']:.0%}  n={r['appearances']}")
else:
...
Every crypto frame is a NO-GO, and the reason changed on 7 September 2026. For a year the obstacle was read as the fee. It is not. The edge diagnostics run that day stripped the fee out entirely and both surviving frames still lost money. On four-hour bars the model's chosen trades returned minus 0.182 per cent each before any cost, against a one-bar persistence rule that returned minus 0.118 per cent, so the model was worse than the crudest possible baseline. On daily bars it returned minus 1.150 per cent. Both records carry the same line, that the problem is the features and the architecture rather than the costs.

The rest of the picture is unchanged. On the one-hour frame the label sweeps, the seven-model zoo and the Monte Carlo gate all land NO-GO with an area under the curve near 0.50, and the default label has negative expectancy before any prediction, a base rate near 0.313 against a breakeven near 0.333. The four-hour frame carries slightly more, 0.549 against 0.50, and the daily and weekly context lifts it further without crossing the fee. The five-minute frame is ruled out by arithmetic, because the moves are smaller than 0.2 per cent a round trip. None of this is a leakage artefact; the leakage audit was read line by line and is clean.

### 5.2 What Tuning Showed

Tuning has been run once, on 6 September 2026, and the result is that tuning is not where the answer is.

Nine settings were tried on the four-hour trend-life target over three walk-forward folds. All nine were rejected. The lowest overfit ratio in the grid was 1.272 against a bar of 1.1, and the highest was 2.339. Across the grid the training error nearly halved, from 18.994 bars down to 10.772, while the held-out error moved the wrong way, from 24.159 up to 25.526. That is the shape of a grid buying memorisation. The setting that scored best on held-out data was the one that fitted its training data worst, a learning rate of 0.03 at a depth of 4.

The second reading matters more. Inside any one setting the three folds land between 3.6 and 4.2 bars apart. Across the whole grid, best setting to worst, the span is 1.366 bars. Which stretch of market a model is asked about moves its score about three times further than any choice made about the model. A leaderboard of settings read without its folds is reading noise and calling it a ranking.

Two defects in the judging were found and fixed on 8 September. Selection took the lowest held-out error without consulting the overfit ratio, which crowned a model at 1.840 that the house rule rejects, then scored the blind year with it. And the tuner never computed a training error at all, so no overfit ratio existed for a tuning grid and the rule could not be applied to one. Both now check the bar before reading the ranking, and both say so plainly when nothing passes.

A separate experiment closes the door on a richer model. Ninety engineered features scored 24.213 bars on the same target. The raw last twenty-four bars, with nothing built from them at all, scored 24.247. The two representations sit 0.034 bars apart, so they carry the same information and a memory-based model has no gap left to close.

### 5.3 Which Features

The record is `04-outputs/AA-evals/2026-09-08/feature-report-20260908.md`, computed on 40,000 four-hour training rows to 21 June 2025, ninety features, base rate 0.262. Every number below is from the training window; the blind year was not opened. Three measures are reported, because none is honest alone. A feature's own area under the curve says how well it ranks winners above losers by itself. Its stability says in how many of five walk-forward folds it pointed the same way it did over the whole window. Its shuffle cost says how much worse the model scores when that one column is scrambled, which catches a feature that is useless alone and valuable beside others.

**What works is the market, not the coin.** The strongest family by a wide margin is relative strength against bitcoin, `f_btc_`, at a shuffle cost of 0.0069. The next family down is worth a third of that. Four of the model's ten most-used columns are in it, led by `f_btc_mom_168`, momentum against bitcoin over a week, at 0.0181, more than twice any other single feature. This is the one family that is not derived from the coin's own price, and it is the family the model leans on hardest. That is the same finding the cross-sectional work reached from the other direction, arriving now as a property of the model rather than a property of a backtest.

**What works next is the classical indicators, and they are the steadiest things in the set.** The pandas-ta block, `f_ta_pta_`, held its direction in five folds out of five, every feature, and carries the largest standalone signal of any family at 0.0486. The in-house indicator block is close behind at 0.0444 and 0.90 stable. Neither is strong enough to trade, but both behave, which is more than most of the set does.

**What fails is the candlestick patterns.** Eight of the ten columns the model does not read at all are candle patterns from the TA-Lib block. `f_tl_cdl_three_black` scores 0.0000 on its own, meaning it is exactly a coin flip, and holds its direction in one fold out of three. Shuffling any of the sixteen unused columns changes the model's score by less than one part in a hundred thousand. The whole TA-Lib family, fifteen features, is the second-weakest in the set.

**What fails more importantly is the adaptive Supertrend.** `f_mst_` was recorded in June 2026 as the most robust cross-sectional signal in the search. On this measure it is the least stable family in the set at 0.60, and second from bottom on shuffle cost at 0.0004. Its flip indicator, `f_mst_flip`, is one of the columns the model does not read. That does not overturn the cross-sectional finding, which was measured differently, but it does mean the signal is not surviving contact with a model that has ninety other columns to choose from.

**Fourteen of the ninety features held their direction in fewer than three folds out of five.** Those describe a period rather than a market, and they are the mechanism by which a grid search buys memorisation. Read alongside 5.2 they explain it: with a sixth of the inputs changing sign between folds, a deeper tree has plenty to fit and nothing to learn.
...
| `03-inputs/model_assessment_1h.py` | the caret-style scorecard, the model zoo, and the tuner (3.9) |
| `03-inputs/sweep_label_1h.py` | the label-geometry sweep (3.8) |
| `03-inputs/monte_carlo_1h.py` | the Monte Carlo robustness gate (3.10) |
| `03-inputs/edge_diagnostics.py` | the Q1, Q5, and Q6 diagnostics and the out-of-sample selectivity test (3.11) |
| `03-inputs/regime_conditioning.py` | the regime-conditioning ablation (3.12) |
...
| `03-inputs/baseline_supertrend_1h.py` | the triple-Supertrend after-fee baseline (1.x, 3.10) |
| `03-inputs/supertrend.py` | the live bot's triple-Supertrend rule |
...
| `03-inputs/flow_data.py` | the trade-flow imbalance data |
| `03-inputs/config.py` | operator configuration and the `LIVE_TRADING` switch |
| `01/02/03-trader-*.ipynb` | the source chapters this notebook consolidates |
| `03-inputs/cross_sectional_regime.py` | the regime-gated cross-sectional edge matrix (3.13, 5.1) |
| `03-inputs/edge_attribution.py` | per-coin attribution of the gated edge (3.13) |
| `03-inputs/narrow_book.py` | the evidence-based tradeable whitelist (3.13) |
| `03-inputs/candidate_screen.py` | the live fee-adjusted range screen (3.13) |
| `03-inputs/fetch_funding.py`, `03-inputs/funding_features.py` | the funding archive and the `f_fund_` crowding gate (3.13) |
| `03-inputs/portfolio_backtest.py` | the gated composite portfolio backtest (3.13) |
