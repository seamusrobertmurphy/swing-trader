...
The sweep is unambiguous and it is the reason tuning is not the lever. Across the six settings the in-sample error fell by a factor of five while the cross-validated error rose monotonically, so the configuration that fits its own training data almost perfectly is the worst in the grid on data it has not seen. Every setting was rejected on the 1.1 bar, and the whole grid spans 0.033 on held-out error against a fold-to-fold spread of 3.83 bars on the trend-life target, which is roughly forty times larger. The fold count is itself a larger lever than any hyperparameter in the grid: three folds return 0.4840 on the leading configuration and five return 0.4894.

The assessment reaches the same conclusion from the other side. Logistic regression and the random forest are the only estimators whose ratio sits inside the bar, at 1.011 to 1.069; the two gradient boosters are rejected at 1.720 and 1.840. Selection now runs inside the passing set, which on the 6 September numbers changes the pick from HistGBM at 1.840 to the random forest at 1.069.

Calibration is the one place where a large improvement was available and it was not an edge. Raw probabilities on the four-hour frame carry an expected calibration error of 0.2344, and Platt scaling reduces it to 0.0295 and the Brier score from 0.2586 to 0.1913. Refitting unweighted, without `class_weight="balanced"`, reaches 0.0736 before any mapping, so that single argument accounted for roughly two thirds of the miscalibration on the metric these models are ranked by. Resolution is 0.00092, near zero, which is the honest reading: the mapping corrects what a probability means without adding information, and the after-fee out-of-sample scoreboard remains the test.
...
Two things were never tried on it. Holding longer, so the fee is paid fewer times, and running the same ranking on shares, where the market drifts up rather than down. The second of those is section 6.5.
