...
def folds_of(n: int, k: int, scheme: str, repeats: int = 10,
    ...
    if scheme in ("expanding", "rolling"):
        ...
        # The training share, caret's trainControl(p = 0.75), "for leave-group
        # out cross-validation: the training percentage" (CRAN reference
        # manual, page 169). It was written here as the constant 0.75 until
        # 21 September 2026, so the one number deciding how much a random split
        # trains on could not be seen on the board or moved.
        for _ in range(max(repeats, 2)):
            order = rng.permutation(n)
            cut = int(n * min(max(float(train_fraction), 0.05), 0.95))
...
def auto_grid(model: str, length: int) -> dict:
    """A grid chosen for the model, from one number, the way caret does it.

    caret: tuneLength is "An integer denoting the amount of granularity in the
    tuning parameter grid. By default, this argument is the number of levels
    for each tuning parameters that should be generated by train."
    (05-research/research/caret-package.pdf, page 165.)

    Writing a grid by hand is the part of tuning that stops a reader tuning at
    all, so this asks for one number instead: how many values to try for each
    setting. The ranges are the ones this repository has measured on, not
    caret's, because caret's models are R's and these are scikit-learn's. Zero
    means the grid typed on the panel is used instead, which is the behaviour
    everything before 21 September 2026 had.
    """
    if length <= 0:
        return {}
    n = max(2, min(int(length), 12))

    def span(lo, hi, cast=float, log=False):
        if log:
            vals = np.geomspace(lo, hi, n)
        else:
            vals = np.linspace(lo, hi, n)
        return sorted({cast(round(v, 6) if cast is float else cast(v)) for v in vals})

    key = model.lower().split(".")[0]
    if key in ("histgbm", "lightgbm", "gbm"):
        return {"learning_rate": span(0.01, 0.2, float, log=True),
                ("max_leaf_nodes" if key == "histgbm" else
                 "num_leaves" if key == "lightgbm" else "max_depth"):
                    span(3, 63, int, log=True) if key != "gbm" else span(2, 8, int)}
    if key == "rf":
        return {"max_depth": span(3, 16, int),
                "min_samples_leaf": span(10, 400, int, log=True)}
    if key.startswith("logreg"):
        return {"C": span(0.001, 10.0, float, log=True)}
    return {}


def choose_winner(rows: list[dict], cfg: dict) -> tuple[dict | None, str]:
    """Which fit the record calls the winner, and the rule that chose it.

    caret makes this a setting: trainControl(selectionFunction = "best"), "the
    function used to select the optimal tuning parameter", with best, oneSE and
    tolerance offered (05-research/research/caret-package.pdf, page 169). This
    board chose on the lowest held-out error and nothing else, while already
    applying the one-standard-error idea to the elastic net on the Choose
    Screen tool, so the same reasoning was used in one place and not the other.

    best   the lowest cross-validated error among the fits that pass the
           overfit bar. The bar is checked first because selecting on held-out
           error alone crowned a model at a ratio of 1.840 on 6 September 2026,
           which the house rule rejects outright, and then scored the blind
           year with it.
    oneSE  the simplest fit whose error is within one standard error of the
           best. Simplest here is the fewest folds' worth of spread, measured
           as the fit with the smallest in-sample error advantage over its
           cross-validated error, which is the least it leaned on memorising.
    """
    passing = [r for r in rows if not r.get("rejected")]
    pool = passing or rows
    if not pool:
        return None, "nothing was scored"
    best = min(pool, key=lambda r: r["cv"]["rmse"])
    rule = str(cfg["split"].get("selection") or "best")
    if rule != "oneSE" or len(pool) < 2:
        return best, ("the lowest held-out error among the fits that passed the "
                      "overfit bar" if passing else
                      "the lowest held-out error; every fit was rejected")
    # One standard error of the best fit's own fold-to-fold spread.
    u = best.get("fold_u2") or []
    if len(u) < 2:
        return best, ("the lowest held-out error; too few folds to measure a "
                      "standard error, so the one-standard-error rule could not run")
    se = float(np.std(u, ddof=1) / np.sqrt(len(u))) * best["cv"]["rmse"]
    within = [r for r in pool if r["cv"]["rmse"] <= best["cv"]["rmse"] + se]
    pick = min(within, key=lambda r: r["rmse_ratio"])
    return pick, (f"the fit that leaned least on memorising, among those within "
                  f"one standard error ({se:.4f}) of the lowest held-out error")
...
def write_record(cfg, rows, screen, cal, label, log=print,
    ...
    if rows:
        ...
        winner, why = choose_winner(rows, cfg)
        if winner is not None:
            L += ["", f"**Chosen: {winner['model']}**"
                      + (" " + " ".join(f"{k}={v}" for k, v in winner["params"].items())
                         if winner["params"] else "")
                      + f", by {why}. caret calls this the selectionFunction and "
                        f"this run used `{cfg['split'].get('selection', 'best')}`.", ""]
...
def main() -> int:
    ...
    if tune:
        grid = auto_grid(tune, int(cfg["model"].get("tune_length") or 0))
        if not grid:
            grid = bc.grid_of(cfg)
        if not grid:
