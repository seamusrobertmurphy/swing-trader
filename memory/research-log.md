...
## 2026-08-18 Track A: A2 complete, A3 built, A4 in flight

Phase A2 landed: 12,539 active US equities enumerated on the SIP consolidated feed, 2,673 pass the screen (median 30d dollar volume >= 20M, price >= $3), and all 2,673 maximum-history adjusted daily-bar files downloaded (5.2 GB, 2016-01-04 onward; universe snapshot inputs/alpaca-data/universe_2026-08-18.csv). Phase A3's adapter (inputs/build_dataset_equity.py) runs the shared frame builder unchanged with SPY in the market seat, an equity ATR band (1-8%), a trading-calendar gap gate, and the swept +3/-1 ATR 20-day label; validated on SPY+AAPL (96 features, base rate 0.304). One exFAT bite: AppleDouble ._ files masqueraded as parquet symbols and crashed the first full build; lister now filters them and unreadable files skip loudly. Phase A4's driver (inputs/equity_edge_matrix.py) ranks the top DECILE at a 50-name floor under a SPY+breadth gate at 0.05% cost with a 0.10% stress column. Full build plus three matrix passes running.

## 2026-08-18 Track A first full read (A4)

Full equity dataset built: 2,547 names, 4,996,861 rows, 96 features, base rate 0.253 (dataset_eq1d_allmarket.parquet, 1.4 GB, gitignored). Records: outputs/AA-evals/2026-08-18/equity-edge-matrix-20260818.md and equity-walkforward-20260818.md.

- The fee wall is no longer the binding constraint. Under the SPY+breadth gate at 0.05% cost the best OOS top-decile is +0.837%/trade on 19,864 trades and nearly every candidate clears zero; but the gated market itself is +0.322%, and beta plus survivorship bias can produce that, so the deciding question moved to the top-minus-market SPREAD.
- The walk-forward kill harness (K1a absolute, K1b selection, both 60% over half-year folds) killed both first candidates: test-picked f_hr_ema_fast_mid at 40%/30% (its blind-year spreads of +0.63/+0.54 were its best regime in a decade), and train-picked f_mo_mom_10 (nearest the canonical momentum factor) at 60%/45%, killed on selection only.
- Structural read: equity attribution is BROAD (964/2118 and 836/1774 names positive) against crypto's narrow concentration, and absolute after-cost economics are positive. The remaining problem is selection stability, not costs. No false GO was issued.
- Named next steps: (1) nested candidate selection (pick signals on train folds only), (2) true 12-1 monthly-rebalance momentum with non-overlapping holds instead of the 20-bar overlapping label, (3) quality/low-vol factor columns, (4) a delisting-complete source to remove the survivorship asterisk before any deployment decision.

## 2026-08-18 first SURVIVES: monthly 12-1 momentum

The canonical 12-1 cross-sectional momentum factor, pre-registered (nothing selected on any score of ours), non-overlapping monthly holds, top decile at a 50-name floor, full-turnover cost bound of 0.05%/month, survives both kill bars on 115 months of the 2,574-name panel. Record: outputs/AA-evals/2026-08-18/monthly-factors-20260818.md.

- Top decile +2.215%/month vs universe +1.132%: spread +1.082%/month, t-stat 2.53, monthly hit rate 60%. Absolute positive in 89% of half-year folds, selection spread positive in 79%, both over the 60% bar. First SURVIVES verdict the scoreboard has ever issued.
- The secondaries behaved exactly as the literature predicts, which is itself evidence the harness measures what it claims: 6-1 momentum weaker (selection 58%, killed by a hair), long-only low-vol negative spread in a bull-heavy sample but positive in the crash halves (2018H2 +0.91, 2022H1 +1.50), its documented defensive shape.
